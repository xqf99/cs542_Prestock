...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 16 20:01:48 2017
...
for i in range(1,11):
    for j in range(1,32):
        date_str = str(i) + '/' + str(j) + '/2017'
        date.append(date_str)
        start_m = str(i)
        start_d = str(j)
        url = url_1 + 'bitcoin' + url_2 + start_m + url_3 + start_d + url_3 + '2017' + url_4 + start_m + url_3 + start_d + url_3 + '2017' + url_5
...
#label[1] = result
#print(label)
#label = np.hstack((date,result))
dataframe = pd.DataFrame({'date':date,'result':result})
dataframe.to_csv("2017.csv",index = False)
'''
